...
static PyXIRNode *lower_sequence(PyXIROp op, asdl_expr_seq *values)
{
    ...
    if (ir_set_children(node, count) < 0) {
        ir_free_node(node);
        return NULL;
    }

    for (Py_ssize_t i = 0; i < count; i++) {
        expr_ty item = (expr_ty)asdl_seq_GET(values, i);
        node->children[i] = lower_expr(item);
        if (!node->children[i]) {
            ir_free_node(node);
            return NULL;
        }
...
static PyXIRNode *lower_dict(expr_ty expr)
{
    ...
    /* Dictionary children are [key, value, key, value, ...].
       A NULL key represents **value, matching the AST's NULL key marker. */
    if (ir_set_children(node, count * 2) < 0) {
        ir_free_node(node);
        return NULL;
    }
    ...
    for (Py_ssize_t i = 0; i < count; i++) {
        ...
        if (key == NULL) {
            PyErr_SetString(PyExc_NotImplementedError,
                            "PythonX: dictionary unpacking is not yet lowered");
            ir_free_node(node);
            return NULL;
        }

        node->children[i * 2] = lower_expr(key);
        node->children[i * 2 + 1] = lower_expr(value);
        if (!node->children[i * 2] || !node->children[i * 2 + 1]) {
            ir_free_node(node);
            return NULL;
        }
    }
...
static PyXIRNode *lower_expr(expr_ty expr)
{
    ...
    if (expr->kind == List_kind) {
        return lower_sequence(PYX_IR_LIST, expr->v.List.elts);
    }

    if (expr->kind == Tuple_kind) {
        return lower_sequence(PYX_IR_TUPLE, expr->v.Tuple.elts);
    }

    if (expr->kind == Set_kind) {
        return lower_sequence(PYX_IR_SET, expr->v.Set.elts);
    }

    if (expr->kind == Dict_kind) {
        return lower_dict(expr);
    }
    ...
    if (expr->kind == BoolOp_kind) {
        ...
        if (n < 1) {
            PyErr_SetString(PyExc_SyntaxError, "empty boolean expression");
            return NULL;
        }
        PyXIRNode *root = lower_expr(
            (expr_ty)asdl_seq_GET(expr->v.BoolOp.values, 0));
        if (!root) return NULL;
        for (Py_ssize_t i = 1; i < n; i++) {
            PyXIRNode *item = lower_expr(
                (expr_ty)asdl_seq_GET(expr->v.BoolOp.values, i));
            if (!item) {
                ir_free_node(root);
                return NULL;
            }
    ...
    if (expr->kind == Compare_kind) {
        if (asdl_seq_LEN(expr->v.Compare.ops) != 1 ||
            asdl_seq_LEN(expr->v.Compare.comparators) != 1) {
            PyErr_SetString(PyExc_NotImplementedError,
                            "chained comparisons are not yet lowered");
            return NULL;
        }
        PyXIROp op;
        ...
        node->right = lower_expr(
            (expr_ty)asdl_seq_GET(expr->v.Compare.comparators, 0));
        if (!node->left || !node->right) {
            ir_free_node(node);
            return NULL;
        }
    ...
    PyErr_Format(PyExc_NotImplementedError,
                 "PythonX IR: unsupported AST expression kind %d",
                 (int)expr->kind);
    return NULL;
}
...
{
    if (!module || !function) {
        PyErr_SetString(PyExc_TypeError, "invalid PythonX IR input");
        return -1;
    }
    function->root = NULL;
    if (module->kind != Module_kind ||
        asdl_seq_LEN(module->v.Module.body) != 1) {
        PyErr_SetString(PyExc_NotImplementedError,
                        "PythonX bootstrap currently accepts one expression statement");
        return -1;
    }
    stmt_ty statement = (stmt_ty)asdl_seq_GET(module->v.Module.body, 0);
    if (statement->kind != Expr_kind) {
        PyErr_SetString(PyExc_NotImplementedError,
                        "PythonX bootstrap currently accepts an expression statement");
        return -1;
    }
...
static PyObject *eval_node(const PyXIRNode *node)
{
    ...
    if (node->op == PYX_IR_LIST || node->op == PYX_IR_TUPLE ||
        node->op == PYX_IR_SET) {
        PyObject *result = NULL;

        if (node->op == PYX_IR_LIST) {
            result = PyList_New(node->child_count);
        } else if (node->op == PYX_IR_TUPLE) {
            result = PyTuple_New(node->child_count);
        } else {
        ...
        for (Py_ssize_t i = 0; i < node->child_count; i++) {
            PyObject *item = eval_node(node->children[i]);
            if (!item) {
                Py_DECREF(result);
                return NULL;
            }
    ...
    if (node->op == PYX_IR_DICT) {
        ...
        for (Py_ssize_t i = 0; i < node->child_count; i += 2) {
            PyObject *key = eval_node(node->children[i]);
            PyObject *value = eval_node(node->children[i + 1]);
            if (!key || !value) {
                Py_XDECREF(key);
                Py_XDECREF(value);
                Py_DECREF(result);
    ...
    if (node->op == PYX_IR_AND || node->op == PYX_IR_OR) {
        PyObject *left = eval_node(node->left);
        if (!left) return NULL;
        ...
        if ((node->op == PYX_IR_AND && !truth) ||
            (node->op == PYX_IR_OR && truth)) {
            return left;
        }
        Py_DECREF(left);
    ...
    PyObject *a = eval_node(node->left);
    if (!a) return NULL;
    ...
    PyObject *r = NULL;
    switch (node->op) {
        case PYX_IR_ADD: r = PyNumber_Add(a, b); break;
        case PYX_IR_SUB: r = PyNumber_Subtract(a, b); break;
        case PYX_IR_MUL: r = PyNumber_Multiply(a, b); break;
        case PYX_IR_MATMUL: r = PyNumber_MatrixMultiply(a, b); break;
        case PYX_IR_DIV: r = PyNumber_TrueDivide(a, b); break;
        case PYX_IR_FLOORDIV: r = PyNumber_FloorDivide(a, b); break;
        case PYX_IR_MOD: r = PyNumber_Remainder(a, b); break;
        case PYX_IR_POW: r = PyNumber_Power(a, b, Py_None); break;
        case PYX_IR_LSHIFT: r = PyNumber_Lshift(a, b); break;
        case PYX_IR_RSHIFT: r = PyNumber_Rshift(a, b); break;
        case PYX_IR_BITOR: r = PyNumber_Or(a, b); break;
        case PYX_IR_BITXOR: r = PyNumber_Xor(a, b); break;
        case PYX_IR_BITAND: r = PyNumber_And(a, b); break;
        case PYX_IR_INVERT: r = PyNumber_Invert(a); break;
        case PYX_IR_POSITIVE: r = PyNumber_Positive(a); break;
        case PYX_IR_NEGATIVE: r = PyNumber_Negative(a); break;
        case PYX_IR_NOT: {
            int t = PyObject_IsTrue(a);
            if (t >= 0) r = PyBool_FromLong(!t);
            break;
        }
        case PYX_IR_LT: r = PyObject_RichCompare(a, b, Py_LT); break;
        case PYX_IR_LE: r = PyObject_RichCompare(a, b, Py_LE); break;
        case PYX_IR_EQ: r = PyObject_RichCompare(a, b, Py_EQ); break;
        case PYX_IR_NE: r = PyObject_RichCompare(a, b, Py_NE); break;
        case PYX_IR_GT: r = PyObject_RichCompare(a, b, Py_GT); break;
        case PYX_IR_GE: r = PyObject_RichCompare(a, b, Py_GE); break;
        case PYX_IR_IS: r = PyBool_FromLong(a == b); break;
        case PYX_IR_IS_NOT: r = PyBool_FromLong(a != b); break;
        case PYX_IR_IN: {
            int t = PySequence_Contains(b, a);
            if (t >= 0) r = PyBool_FromLong(t);
            break;
        }
        case PYX_IR_NOT_IN: {
        ...
        default:
            PyErr_SetString(PyExc_NotImplementedError,
                            "unsupported PythonX IR operation");
            break;
    }
...
{
    if (!node) {
        PyErr_SetString(PyExc_RuntimeError, "null PythonX IR node");
        return NULL;
    }
