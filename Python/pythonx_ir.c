...
static PyXIRNode *lower_expr(expr_ty e)
{
    ...
    case NamedExpr_kind:n=ir_new(PYX_IR_NAMED_EXPR);if(!n||set_children(n,2)<0){ir_free_node(n);return NULL;}n->children[0]=lower_store(e->v.NamedExpr.target, none_node());n->children[1]=lower_expr(e->v.NamedExpr.value);if(!n->children[0]||!n->children[1]){ir_free_node(n);return NULL;}return n;
